package client.gui;

import java.awt.Color;

public class UIResourcesDark extends UIResources
{
	
// PUBLIC METHOD

	// Constructor
	private UIResourcesDark()
	{
		super();
	}
	
// PUBLIC RESOURCES

	// Colors
	public static final Color BACKGROUND_COLOR = new Color(0x2B332D),
							  USER_MESSAGE_COLOR = new Color(0x539664),
							  OTHER_MESSAGE_COLOR = new Color(0x8E9E92),
							  PANEL_CONNECT_FONT_LABEL_COLOR = new Color(0xFFFFFF),
							  PANEL_CONNECT_FONT_FIELD_COLOR = new Color(0x333333),
							  TREE_COLOR = new Color(0x8E9E92),
							  FONT_TREE_COLOR = new Color(0x333333),
							  TREE_PSEUDO_COLOR = new Color(0xBF270F);
	
}
